...
area_data = {
    ...
    '广西':['广西区', '贵港市', '玉林市', '北海市', '南宁市', '柳州市', '桂林市', '梧州市', '钦州市', '来宾市', '河池市', '百色市', '贺州市', '崇左市',  '防城港市'],
    ...
    '黑龙江':['黑龙江省', '齐齐哈尔市', '哈尔滨市', '大庆市', '佳木斯市', '双鸭山市', '牡丹江市', '鸡西市','黑河市', '绥化市', '鹤岗市', '伊春市', '大兴安岭地区', '七台河市'],
    ...
    '新疆':['新疆区','新疆维吾尔自治区', '乌鲁木齐市', '伊犁州', '昌吉州','石河子市', '哈密地区', '阿克苏地区', '巴音郭楞州', '喀什地区', '塔城地区', '克拉玛依市', '和田地区', '阿勒泰州', '吐鲁番地区', '阿拉尔市', '博尔塔拉州', '五家渠市',
     '克孜勒苏州', '图木舒克市'],
    '西藏':['西藏区', '拉萨市', '山南地区', '林芝地区', '日喀则地区', '阿里地区', '昌都地区', '那曲地区'],
    '吉林':['吉林省', '吉林市', '长春市', '白山市', '白城市','延边州', '松原市', '辽源市', '通化市', '四平市'],
    '宁夏':['宁夏区', '银川市', '吴忠市', '中卫市', '石嘴山市', '固原市'] }


df=pd.read_excel('知乎用户.xlsx')
x=df.dropna(axis=0,subset=['所在地'])
y=x['所在地']
z=list(y.values)
d={}
for i in z:
    for j,k in area_data.items():
        for p in k:
            q=p[:-1]
            if q in i:
                if j in d:
...
map = (
    Map(init_opts=opts.InitOpts(bg_color="#FFFAFA", theme=ThemeType.ESSOS, width=1000))
        .add("人数", d.items())
        .set_global_opts(
        title_opts=opts.TitleOpts(title="中国知乎用户分布图",pos_left=20),
        visualmap_opts=opts.VisualMapOpts(
            is_piecewise=True,  # 设置是否为分段显示
            # 自定义的每一段的范围，以及每一段的文字，以及每一段的特别的样式。例如：
            pieces=[
                {"min": 4000, "label": '>4000人', "color": "#eb2f06"},
                {"min": 2000, "max": 4000, "label": '2001-4000人', "color": "#FF3030"},  # 不指定 max，表示 max 为无限大（Infinity）。
                {"min": 1000, "max": 2000, "label": '1001-2000人', "color": "#FF4500"},
                {"min": 500, "max": 1000, "label": '501-1000人', "color": "#FF7F50"},
                {"min": 100, "max": 500, "label": '100-500人', "color": "#FFA500"},
                {"min": 1, "max": 100, "label": '1-99人', "color": "#FFDEAD"},
            ],
            # 两端的文本，如['High', 'Low']。
            range_text=['高', '低'],
        ),
...
map.render('中国知乎用户分布图.html')

#知乎用户人数Top10
y=sorted(d.items(),key=lambda k: -k[1])[:10]
d=dict(y)
areas=list(d.keys())
...
keywords_count_bar = (
    Bar(init_opts=opts.InitOpts(theme=ThemeType.ROMANTIC))
        ...
        title_opts=opts.TitleOpts(title="知乎用户人数Top10"),
        yaxis_opts=opts.AxisOpts(name="省份"),
        xaxis_opts=opts.AxisOpts(name="人数")
        )
    )
keywords_count_bar.render('知乎用户人数.html')

#性别比例
a=df.groupby('性别')['性别'].count()
